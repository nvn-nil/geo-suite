...
def get_file_options(input_item, args):
    ...
    return {
        ...
        "thinning_options": {
            "allowed_angle_deviation": args.allowed_angle_deviation if args.allowed_angle_deviation else config_data.get("allowed_angle_deviation", DEFAULT_ALLOWED_ANGLE_DEVIATION),
            "use_weighted_tolerance": args.use_weighted_tolerance if args.use_weighted_tolerance else config_data.get("use_weighted_tolerance", DEFAULT_USE_WEIGHTED_TOLERANCE),
            "max_distance_for_weighting": args.max_distance_for_weighting if args.max_distance_for_weighting else config_data.get("max_distance_for_weighting", DEFAULT_MAX_DISTANCE_FOR_WEIGHTING),
            "max_angle_for_weighting": args.max_angle_for_weighting if args.max_angle_for_weighting else config_data.get("max_angle_for_weighting", DEFAULT_ANGLE_FOR_WEIGHTING),
        }
    }
...
def main():
    ...
    parser.add_argument("--use-distance-weighted", dest="use_weighted_tolerance", help='<Optional> Smaller distance between the points allow for higher angle deviation', action="store_true")
    parser.add_argument("--allowed-angle-deviation", help="<Optional> Angular deviation allowed from straight line", type=float)
    parser.add_argument(
    ...
    parser.add_argument("--max-angle-for-weighting", help="<Optional> When using distance weighted, this will be the maximum angle for distance close to zero", type=float)
    parser.add_argument(
        "-s",
        "--output-suffix",
